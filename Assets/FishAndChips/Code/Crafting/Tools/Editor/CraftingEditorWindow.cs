			...
			foreach (var guid in craftItemGUIDS)
			{
				...
				if (asset != null)
				{
					if (_craftItems.Contains(asset) == false)
					{
						...
						{
							if (asset.CraftItemModelData.VisualKey.IsNullOrEmpty() == false)
							{
								var textureGUIDs = AssetDatabase.FindAssets($"t:Texture2D {asset.CraftItemModelData.VisualKey}", new[] { $"Assets/FishAndChips/Art/Sprites/CraftItemIcons" });
								if (textureGUIDs != null && textureGUIDs.Length > 0)
								{
									var texturePath = AssetDatabase.GUIDToAssetPath(textureGUIDs[0]);
